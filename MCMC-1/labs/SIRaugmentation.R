...
initializedata = function(remtimes) {
	...
	for (k in 2:n) {
		inftimes[k]      = inftimes[k - 1] + (remtimes[k - 1] - inftimes[k - 1]) /
			2
...
loglikelihood = function(A, M, bet, gam) {
	...
	#          This agrees with the actual augmentation routine
	#          (update_inftimes.R),
	...
	# The number of infected individuals during the entire outbreak,
	...
	for (i in 2:n) {
		...
		# The number of infected individuals at the time of the infection in
		# question
...
totaltime_infpressure = function(completedata, M) {
	...
	# the outbreak
	# The total time spent infective
	inftime   = totaltime_infected(completedata)
...
update_beta = function(completedata, M) {
	...
	# parameters of the full conditional (Beta) distribution of parameter 'beta'
...
sampleSIR = function(remtimes, M, mcmc.size) {
	...
	# initialize the complete data matrix with 2 columns: infection times and
	# removal times
	...
	for (i in 2:mcmc.size) {
		...
		bet          = update_beta(completedata, M)
		gam          = update_gamma(completedata)
		# (e) unobserved infection times
		completedata = update_inftimes(completedata, M, bet, gam) 
...
par(mfrow = c(1, 2))
plot(mcmc.sample$beta,
		 type = "l",
		 xlab = "iteration",
		 ylab = "beta")
plot(mcmc.sample$gam,
		 type = "l",
		 xlab = "iteration",
		 ylab = "gamma")
...
par(mfrow = c(1, 1))
hist(retain.sample$bet / retain.sample$gam,
		 main = "",
		 xlab = 'R0 (=beta/gamma)')
