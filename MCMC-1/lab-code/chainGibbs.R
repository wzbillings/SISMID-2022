################################################################################
# Example Gibbs sampler for chain binomial model
###
...
# Updated July 18, 2022 / WZB
################################################################################

# 'model built in sampler' ####
chainGibbs <- function(mcmc.size, alpha, beta){
	...
	# Just an initial guess for q
	q[1]    = 0.5
	# Rounded expected value of n_111 given q = 0.5
	n111[1] = round(275*2*q[1]/(2*q[1]+1))
	
	# The observations (cf. the lecture)
	n1  = 34   # frequency of chain 1
	n11 = 25   # frequency of chain 1->
	# frequency of chains with outbreak size 3 (the total frequency of chains
	# 1->2 and 1->1->1)
	N3  = 275  
	...
	for (i in 2:mcmc.size){
		...
		# we already updated q so we want to use the most recent value of q
...
chainGibbsGeneralized <- function(mcmc.size, n1, n11, N3, alpha, beta) {
	# Pre-allocate space
	q <- rep(0, mcmc.size)
	n111 <- rep(0, mcmc.size)
	...
	# Initialize unknowns
	q[1] <- 0.5
	n111[1] <- round(N3 * 2 * q[1] / (2 * q[1] + 1))
	
	# Draw samples
	for (i in 2:mcmc.size) {
		q[i] <- rbeta(
			1,
			2 * n1 + 2 * n11 + n111[i-1] + alpha,
			n11 + 2 * N3 + beta
		)
	...
	out <- list(q = q, n111 = n111)
	return(out)
}

## Running the generalized sampler ####
test2 <- chainGibbsGeneralized(5000, 34, 25, 275, 1, 1)

# Assume a burn-in of 500 iterations, so just plot those greater than 500. 

hist(test2$q[501:5000],    main = "", xlab = "q")
hist(test2$n111[501:5000], main="", xlab = "n111")

# Summary of the results 
summary(test2$q[501:5000])
summary(test2$n111[501:5000])
