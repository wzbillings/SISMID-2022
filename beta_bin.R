...
#' Propose a positive parameter by multiplying the current value by
#' exp(lambda(U-0.5))
#'
#' @param cur_pospar Current state of M-H Markov chain
#' @param prop_unif Realization of U ~ Unif[0,1]
#' @param tuning_const Tuning parameter lambda that is used in the random
#'   multiplier exp(lambda(U-0.5))
#'
...
#' Compute log of the density (up to proportionality constant) of the proposal
#' distribution that multiplies the current value of a positive parameter by
#' exp(lambda(U-0.5))
#'
...
#'
#' @param my_data data frame with two columns named x and n. The first column
#'   (x) contains the numbers of rats that developed endometrial stromal polyps
#'   and the second column has the total number of rats in each colony. The rows
#'   correspond to different rat colonies
...
#' @param prior_rate_alpha of the exponential prior distribution for
#'   hyperparameter alpha
#' @param prior_rate_beta of the exponential prior distribution for
#'   hyperparameter beta
#'
#' @return value of the log-posterior terms that depend on the hyperparameter
#'   beta
#'
#' @examples log_posterior(1.1, 1.2, rep(0.1,71), rat_data, 71, 0.1, 0.1)
log_posterior = function(my_alpha,
												 my_beta,
												 theta_vec,
												 my_data,
												 sample_size,
	...
	return(
		sample_size *
			(lgamma(my_alpha + my_beta) - lgamma(my_alpha) - lgamma(my_beta)) +
			sum((my_data$x + my_alpha - 1) * log(theta_vec)) +
			sum((my_data$n - my_data$x + my_beta - 1) * log(1 - theta_vec)) -
			prior_rate_alpha * my_alpha - prior_rate_beta * my_beta
	)
}
...
#' @param prior_rate of the exponential prior distribution for hyperparameter
#'   alpha
#'
#' @return value of the log-posterior terms that depend on the hyperparameter
#'   alpha
...
#' and hyperparamters in the beta-binomial model
#'
#' @param my_data data frame with two columns named x and n. The first column
#'   (x) contains the numbers of rats that developed endometrial stromal polyps
#'   and the second column has the total number of rats in each colony. The rows
...
#' @param prior_rate_alpha rate of the exponential prior distribution for
#'   hyperparameter alpha
#' @param prior_rate_beta rate of the exponential prior distribution for
#'   hyperparameter beta
#' @param tuning_alpha tuning parameter for the proposal distribution for
#'   hyperparameter alpha (lambda_alpha in the lab algorithm pseudocode)
#' @param tuning_beta tuning parameter for the proposal distribution for
#'   hyperparameter beta (lambda_beta in the lab algorithm pseudocode)
#' @param mcmc_size number of MCMC iterations
#' @param mcmc_burnin number of initial MCMC iterations to discard
#' @param mcmc_subsample number of MCMC iterations to skip over when saving
#'   results; e.g., mcmc_subsample=10 says save every 10th iteration
#'
#' @return matrix with rows corresponding to saved MCMC iterations and the
#'   columns recording 1) iteration index; 2) values of the log-posterior; 3)
#'   value of alpha; 4) value of beta; 5) binary indicator of proposed alpha
#'   acceptance; 6) binary indicator of proposed alpha acceptance; 7-77) values
#'   of binomial success probabilities theta[1:71]
#'
#' @examples mcmc_output = mcmc_sampler(my_data=rat_data, init_alpha=1.0,
#' init_beta=1.0, prior_rate_alpha=0.1, prior_rate_beta=0.1, tuning_alpha=0.7,
#' tuning_beta=0.7, mcmc_size=110000, mcmc_burnin=10000, mcmc_subsample=10)
mcmc_sampler = function(my_data,
												init_alpha,
												init_beta,
												prior_rate_alpha,
												prior_rate_beta,
	...
	mcmc_out = matrix(0,
										nrow = (mcmc_size - mcmc_burnin) / mcmc_subsample,
										ncol = data_sample_size + 6)
	colnames(mcmc_out) = c("iter",
												 "posterior",
												 "alpha",
												 "beta",
												 "alpha.acc",
	...
	for (i in 1:mcmc_size) {
		...
		if (i > mcmc_burnin) {
			...
			if (step_count == mcmc_subsample) {
				...
				mcmc_out[my_index, 2] = log_posterior(
					cur_alpha,
					cur_beta,
					cur_theta,
					my_data,
